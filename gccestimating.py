...
class GCC(object):
    ...
    def cc(self):
        ...
        if self._cc is None:
            self._cc = GCC.Estimate(
                name="CC", sig=self._backtransform(self.spec12), spec=self.spec12
            )
    ...
    def roth(self):
        ...
        if self._roth is None:
            ...
            self._roth = GCC.Estimate(
                name="Roth", sig=self._backtransform(spec), spec=spec
            )
        return self._roth
    ...
    def scot(self):
        ...
        if self._scot is None:
            ...
            self._scot = GCC.Estimate(
                name="SCOT", sig=self._backtransform(spec), spec=spec
            )
        return self._scot
    ...
    def phat(self):
        ...
        if self._phat is None:
            ...
            self._phat = GCC.Estimate(
                name="PHAT", sig=self._backtransform(spec), spec=spec
            )
        return self._phat
    ...
    def eckart(self, sig0, noise1, noise2):
        ...
        if self._eckart is None:
            ...
            weight = spec_sig00 / _prevent_zerodivision(spec_noise11 * spec_noise22)
            spec = self.spec12 * weight
            self._eckart = GCC.Estimate(
                name="Eckart", sig=self._backtransform(spec), spec=spec
            )
        return self._eckart
    ...
    def ht(self):
        ...
        if self._ht is None:
            ...
            spec = (
                self.spec12
                * coh
                / _prevent_zerodivision(_np.abs(self.spec12) * (1 - coh))
            )
            self._ht = GCC.Estimate(name="HT", sig=self._backtransform(spec), spec=spec)
        return self._ht
